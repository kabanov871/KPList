...
class SearchByPersonAdapter(private val searchMovie: (movieId: String) -> Unit): RecyclerView.Adapter<SearchByPersonAdapter.SearchByPersonHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SearchByPersonHolder {
        ...
        val binding : SearchByPersonItemBinding = SearchByPersonItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    class SearchByPersonHolder(val binding: SearchByPersonItemBinding): RecyclerView.ViewHolder(binding.root){


        fun bind(model: PreviewByPersonUseCaseModel,
                 searchMovie:(movieId: String)->Unit) {

            binding.apply {

                val getImage = model.movieId
                Picasso.get().load("https://st.kp.yandex.net/images/film_big/${getImage}.jpg").into(imageView4)
